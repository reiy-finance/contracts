...
missing=0
for key in \
  PACKAGE_ID AUCTION_STATE_ID GLOBAL_CONFIG_ID ADMIN_CAP_ID UPGRADE_CAP_ID
do
  if [[ -z "${!key}" ]]; then
...
set_var SOLVER_REGISTRY_ID  ""
set_var FEE_VAULT_ID        ""
if ! grep -q "^COORDINATOR_KEY_VERSION=" "$ENV_FILE" 2>/dev/null; then
  set_var COORDINATOR_KEY_VERSION "1"
fi
